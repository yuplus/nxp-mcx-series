...
#define SIM_LPOCLKS_LPOCLKSEL_VAL ((SIM->LPOCLKS & SIM_LPOCLKS_LPOCLKSEL_MASK) >> SIM_LPOCLKS_LPOCLKSEL_SHIFT)
...
uint32_t CLOCK_GetLpoClkFreq(void)
{
    ...
    switch (SIM_LPOCLKS_LPOCLKSEL_VAL)
    {
        case kSIM_LpoClkSrcLpo128k:
            freq = LPO_128K_CLK_FREQ;
            break;
        case kSIM_LpoClkSrcLpo32k:
            freq = LPO_32K_CLK_FREQ;
            break;
        case kSIM_LpoClkSrcLpo1k:
            freq = LPO_1K_CLK_FREQ;
            break;
        default:
            freq = 0U;
            break;
...
uint32_t CLOCK_GetFreq(clock_name_t clockName)
{
    ...
    switch (clockName)
    {
        ...
        case kCLOCK_Lpo1kClk:
            freq = LPO_1K_CLK_FREQ;
            break;
        case kCLOCK_Lpo32kClk:
            freq = LPO_32K_CLK_FREQ;
            break;
        case kCLOCK_Lpo128kClk:
            freq = LPO_128K_CLK_FREQ;
            break;
        case kCLOCK_LpoClk:
    ...
    if (sircFreq != 0UL)
    {
        switch (type)
        {
            case kSCG_AsyncDiv2Clk: /* SIRCDIV2_CLK. */
                ...
                break;
            case kSCG_AsyncDiv1Clk: /* SIRCDIV1_CLK. */
